...
class MeanReversionStrategyWithSMAandIBS(Strategy):
    ...
    def init(self):
        ...
        self.sma = self.I(lambda: talib.SMA(self.data.Close, timeperiod=self.sma_window), name='SMA')
    ...
    def next(self):
        ...
        # Bull Market Entry (IBS < 0.3 and price below lower band or close crosses sma)
        if self.data.Close[-1] > self.sma[-1] or (self.data.Close[-1] < self.lower_band[-1] and self.ibs[-1] < 0.3):
            if not self.position:
                self.add_buy_trade()
        
        # Exit Strategy if Price Exceeds High or sma crosses close (Mean-Reversion Exit)
        elif self.data.Close[-1] < self.sma[-1] or self.data.Close[-1] > self.rolling_high[-2]:
            if self.position:
                self.position.close()
...
bt = Backtest(GOOG, MeanReversionStrategyWIthSMAandIBS, cash=100000, commission=.002)
stats = bt.run()
bt.plot()
print(stats)
